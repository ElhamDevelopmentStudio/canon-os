import { CONSUMPTION_STATUSES, MEDIA_TYPES, type MediaItem, type MediaItemCreateRequest } from "@canonos/contracts";
...
export function MediaFormModal({
  ...
  return (
    <div aria-labelledby="media-form-title" aria-modal="true" className="fixed inset-0 z-50 overflow-y-auto bg-foreground/40 p-4 backdrop-blur-sm" role="dialog">
      <div className="mx-auto my-8 w-full max-w-3xl rounded-2xl border border-border bg-card p-6 shadow-xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold" id="media-form-title">{title}</h2>
            <p className="mt-1 text-sm text-muted-foreground">Save the core details needed for library search, filtering, and later scoring.</p>
          </div>
          <Button type="button" variant="ghost" onClick={onClose}>Cancel</Button>
        </div>
        {error ? <div className="mt-4"><ErrorState title="Save failed" message={error} /></div> : null}
        <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            <FormFieldWrapper id="media-title" label="Title">
              <TextInput id="media-title" required value={form.title} onChange={(event) => updateField("title", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-original-title" label="Original title">
              <TextInput id="media-original-title" value={form.originalTitle} onChange={(event) => updateField("originalTitle", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-type" label="Media type">
              <select className={selectClassName} id="media-type" value={form.mediaType} onChange={(event) => updateField("mediaType", event.target.value as FormState["mediaType"])}>
                {MEDIA_TYPES.map((type) => <option key={type} value={type}>{mediaTypeLabels[type]}</option>)}
              </select>
            </FormFieldWrapper>
            <FormFieldWrapper id="media-status" label="Status">
              <select className={selectClassName} id="media-status" value={form.status} onChange={(event) => updateField("status", event.target.value as FormState["status"])}>
            ...
            <FormFieldWrapper id="media-year" label="Release year">
              <TextInput id="media-year" inputMode="numeric" max="2100" min="1800" type="number" value={form.releaseYear} onChange={(event) => updateField("releaseYear", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-creator" label="Creator / director / author">
              <TextInput id="media-creator" value={form.creator} onChange={(event) => updateField("creator", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-country-language" label="Country / language">
              <TextInput id="media-country-language" value={form.countryLanguage} onChange={(event) => updateField("countryLanguage", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-rating" label="Personal rating">
              <TextInput id="media-rating" inputMode="decimal" max="10" min="0" step="0.1" type="number" value={form.personalRating} onChange={(event) => updateField("personalRating", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-started" label="Started date">
              <TextInput id="media-started" type="date" value={form.startedDate} onChange={(event) => updateField("startedDate", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-completed" label="Completed date">
              <TextInput id="media-completed" type="date" value={form.completedDate} onChange={(event) => updateField("completedDate", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-runtime" label="Runtime minutes">
              <TextInput id="media-runtime" inputMode="numeric" min="0" type="number" value={form.runtimeMinutes} onChange={(event) => updateField("runtimeMinutes", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-episodes" label="Episode count">
              <TextInput id="media-episodes" inputMode="numeric" min="0" type="number" value={form.episodeCount} onChange={(event) => updateField("episodeCount", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-pages" label="Page count">
              <TextInput id="media-pages" inputMode="numeric" min="0" type="number" value={form.pageCount} onChange={(event) => updateField("pageCount", event.target.value)} />
            </FormFieldWrapper>
            <FormFieldWrapper id="media-audio-length" label="Audiobook length minutes">
              <TextInput id="media-audio-length" inputMode="numeric" min="0" type="number" value={form.audiobookLengthMinutes} onChange={(event) => updateField("audiobookLengthMinutes", event.target.value)} />
            </FormFieldWrapper>
          </div>
          <FormFieldWrapper id="media-notes" label="Notes">
            <textarea className="min-h-28 rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none transition placeholder:text-muted-foreground focus:ring-2 focus:ring-primary" id="media-notes" value={form.notes} onChange={(event) => updateField("notes", event.target.value)} />
          </FormFieldWrapper>
          <div className="flex justify-end gap-3">
            <Button type="button" variant="ghost" onClick={onClose}>Cancel</Button>
            <Button disabled={isSaving} type="submit">{isSaving ? "Saving…" : "Save media"}</Button>
          </div>
        </form>
      </div>
    </div>
