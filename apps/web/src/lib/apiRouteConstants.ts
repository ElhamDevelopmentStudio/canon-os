export const API_ROUTES = {
  root: "/",
  health: "/health/",
  authCsrf: "/auth/csrf/",
  authRegister: "/auth/register/",
  authLogin: "/auth/login/",
  authLogout: "/auth/logout/",
  authMe: "/auth/me/",
  authProfile: "/auth/profile/",
  authSettings: "/auth/settings/",
  authDataSummary: "/auth/data/",
  authDataDelete: "/auth/data/delete/",
  authAccountDelete: "/auth/account/",
  aftertaste: "/aftertaste/",
  aftertastePrompts: "/aftertaste/prompts/",
  adaptationRelations: "/adaptations/relations/",
  analyticsOverview: "/analytics/",
  analyticsConsumptionTimeline: "/analytics/consumption-timeline/",
  analyticsRatingDistribution: "/analytics/rating-distribution/",
  analyticsMediaTypeDistribution: "/analytics/media-type-distribution/",
  analyticsDimensionTrends: "/analytics/dimension-trends/",
  analyticsGenericnessSatisfaction: "/analytics/genericness-satisfaction/",
  analyticsRegretTimeCost: "/analytics/regret-time-cost/",
  analyticsTopCreators: "/analytics/top-creators/",
  analyticsTopThemes: "/analytics/top-themes/",
  dashboardSummary: "/dashboard/summary/",
  detoxRules: "/detox/rules/",
  detoxEvaluate: "/detox/evaluate/",
  detoxDecisions: "/detox/decisions/",
  detoxTimeSaved: "/detox/time-saved/",
  mediaItems: "/media-items/",
  globalSearch: "/search/",
  metadataProviders: "/metadata/providers/",
  metadataMatches: "/metadata/matches/",
  narrativeAnalyses: "/narrative-analyses/",
  importBatches: "/imports/",
  importPreview: "/imports/preview/",
  exportRequest: "/exports/",
  exportRestoreDryRun: "/exports/restore-dry-run/",
  backgroundJobs: "/jobs/",
  tasteDimensions: "/taste-dimensions/",
  tasteProfile: "/taste-profile/",
  tasteGraphSummary: "/taste-graph/summary/",
  tasteGraphNodes: "/taste-graph/nodes/",
  tasteGraphEdges: "/taste-graph/edges/",
  tasteGraphRebuild: "/taste-graph/rebuild/",
  tasteEvolution: "/taste-evolution/",
  tasteEvolutionGenerate: "/taste-evolution/generate/",
  candidates: "/candidates/",
  canonSeasons: "/seasons/",
  criticPersonas: "/critic-personas/",
  councilSessions: "/council-sessions/",
  discoveryGenerate: "/discovery/generate/",
  discoveryTrails: "/discovery/trails/",
  antiGenericRules: "/anti-generic/rules/",
  antiGenericEvaluate: "/anti-generic/evaluate/",
  queueItems: "/queue-items/",
  queueRecalculate: "/queue-items/recalculate/",
  tonightMode: "/queue/tonight/",
  schema: "/schema/",
  swaggerDocs: "/docs/swagger/",
  scalarDocs: "/docs/scalar/",
} as const;

export type ApiRouteKey = keyof typeof API_ROUTES;
export type ApiRoutePath = (typeof API_ROUTES)[ApiRouteKey];
